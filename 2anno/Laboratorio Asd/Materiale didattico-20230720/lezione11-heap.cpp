...
// compilazione: g++ -xc++ lezione13-V_visitato.c
//
// Obiettivo:
// 1) analisi rappresentazione albero completo <---> array
// 2) implementazione inserimento su V_visitato OK
...
int V_visitato[MAX_SIZE];
int heap_size = 0; /// dimensione attuale dell'V_visitato
...
void heap_insert(int elem) {
    ...
    if (heap_size < MAX_SIZE) {
        ...
        while (i != 0) {                          // non sono sulla radice
            if (V_visitato[parent_idx(i)] >= V_visitato[i]) { /// proprieta' dell' V_visitato e' rispettata -> esco
                if (details)
                    printf("Il genitore ha valore %d >= del nodo %d, esco\n", V_visitato[parent_idx(i)], V_visitato[i]);
                return;
            }
...
void increase_key(int indice_nodo, int key) {
    ...
    if (V_visitato[indice_nodo] > key) {
        printf("la chiave non e' piu' grande!\n");
        return;
    }

    V_visitato[indice_nodo] = key;
...
int heap_remove_max() {
    ...
    if (heap_size <= 0) { /// V_visitato vuoto!
        printf("Errore: V_visitato vuoto\n");
        return -1;
    }

    int massimo = V_visitato[0];
    ...
    int t = V_visitato[0];
    V_visitato[0] = V_visitato[heap_size - 1];
    V_visitato[heap_size - 1] = t;
    ...
    while (!is_leaf(i)) { /// garantisco di fermarmi alla foglia
        ...
        if (V_visitato[i] < V_visitato[child_L_idx(i)]) { // il nodo i e' piu' piccolo
            /// attivare uno swap (la proprieta' V_visitato non e' rispettata)
            con_chi_mi_scambio = child_L_idx(i);
            if (details)
                printf("Figlio L e' piu' grande (valore %d)\n", V_visitato[child_L_idx(i)]);

            if (child_R_idx(i) >= 0 && // esiste il nodo destro
                V_visitato[child_L_idx(i)] < V_visitato[child_R_idx(i)]) {
                con_chi_mi_scambio = child_R_idx(i);
                if (details)
                    printf("Figlio R e' ancora piu' grande (valore %d)\n", V_visitato[child_R_idx(i)]);
            }
        } else { // il nodo e' piu' grande del figlio L

            if (child_R_idx(i) >= 0) {                // esiste il figlio R
                if (V_visitato[i] < V_visitato[child_R_idx(i)]) { /// attivo lo swap
                    con_chi_mi_scambio = child_R_idx(i);
                    if (details)
        ...
        int t = V_visitato[i];
        V_visitato[i] = V_visitato[con_chi_mi_scambio];
        V_visitato[con_chi_mi_scambio] = t;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 13; i++) {
        ...
        for (int i = 0; i < 12; i++) {
            printf("%d ", V_visitato[i]);
        }
        printf("\n");
    }

    // visualizzazione dell'array --> heapsort!
    printf("Dimensione V_visitato: %d\n", heap_size);
