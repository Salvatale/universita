...
node_t* radice;   //salvo primo nodo fornito dal file
int ft = 0;       //diventa 1 appena creo primo nodo

node_t* build_euler(int preordine){

  int n1,n2,n3;
  n1 = preordine;
  bool sx = 0;  //1 allora ha fatto prima chiamata ricorsiva, 0 invece no
  ...
  node_t* node1 = node_new(n1); //creo il nodo con valore n1
  if(!ft){
    radice = node1;
    ft = 1;
  }
  ...
  // faccio prima chiamata ricorsiva, al termine assegno il nodo
  //  ritornato dalla chiamata come figlio sinistro del mio nodo
  // di partenza
  if (n1!=n2) {
    node_t* node=build_euler(n2);
    node1->L = node;
    sx = 1;
  }
  ...
  //controllo se il nodo principale ha gia fatto una chiamata ricorsiva
  // e il valore di n3 è uguale in modo da sapere per certo che non
  // è un nodo foglia
  if(n3 == n1 && sx){
    input_visit >> n3;
    return node1;
...
void tree_depth(node_t* node,int actual_depth){
  if(node->L != NULL)
    tree_depth(node->L,actual_depth+1);
  if(node->R != NULL)
    tree_depth(node->R,actual_depth+1);
...
*/
  ...
  */
  ...
  int partenza;
  input_visit >> partenza;
  root=build_euler(partenza);
  global_ptr_ref=root;
  input_visit.close();
  ...
  //profondità albero
  tree_depth(root,0);
  cout << "profondità albero: " << depth << endl;
  depth = 0;  //azzero per un nuovo conteggio
