...
int** E;  /// array di puntatori a le liste di adiacenza per ogni nodo
int n_nodi;
...
int* global_ptr_ref=NULL;  /// usato per memorizzare il puntatore alla prima lista allocata
...
void node_print(int n){
  ...
  for(int i=0;i<n_nodi;i++){ /// disegno arco
    if(E[n][i] == 1){
      ...
      output_graph << "node_" << V[i] << "_" << n_operazione <<" [  color=gray ]\n";
    }
  }
...
int main(int argc, char **argv) {
  ...
  E= new int*[n_nodi]; //(list_t**)malloc(n_nodi*sizeof(list_t*));
