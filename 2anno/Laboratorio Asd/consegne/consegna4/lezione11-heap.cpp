...
void heap_insert_min(int elem,int index) {
    ...
    if (heap_size < MAX_SIZE) {
        ...
        while (i != 0) {                          // non sono sulla radice
            ...
            int ind = indexes[parent_idx(i)];
            indexes[parent_idx(i)] = indexes[i];
            indexes[i] = ind;

            int p = position_heap[ind];
            position_heap[ind] = position_heap[index];
            position_heap[index] = p;
...
void decrease_key(int indice_nodo, int key) {
    ...
    while (i != 0) {                          // non sono sulla radice
        ...
        int p = position_heap[ind];
        position_heap[ind] = position_heap[i];
        position_heap[i] = p;
...
int heap_remove_min() {
    ...
    int index = indexes[heap_size - 1];
    int index_0 = indexes[0];
    indexes[heap_size -1] = indexes[0];
    indexes[0] = index;

    position_heap[index] = 0;
    position_heap[index_0] = heap_size - 1;
    ...
    while (!is_leaf(i)) { /// garantisco di fermarmi alla foglia
        ...
        int ind = indexes[con_chi_mi_scambio];
        indexes[con_chi_mi_scambio] = indexes[i];
        indexes[i] = ind;
...
int main(int argc, char **argv) {
    ...
    indexes = new int[N];
    position_heap = new int[N];
    
    for(int i = 0; i < N; i++){
        int elem = rand()%100;
        heap_insert_min(elem,i);
    }
    ...
    for(int i = 0; i < N; i++){
        ...
        cout << "heap[" << i << "] with index: " << indexes[i] << "and value: " << heap[i] << endl;
        cout << "heap[" << i << "] with index: " << indexes[i] << "and value: " << heap[position_heap[indexes[i]]] << endl;
