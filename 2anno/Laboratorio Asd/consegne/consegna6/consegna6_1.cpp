...
//Valore che mi specifica la soglia, gli archi che sono maggiori di 113 - limit allora sono pesanti abbastanza da essere interessanti
//Si può giocare con questa costante per ottenere più percorsi(aumentando) o meno percorsi(diminuendo), questo perchè ho applicato shortest path e quindi
//gli archi più leggeri sono quelli in realtà più pesanti.
const int limit = 93;
...
int DFS_weight_path(int n, int w) {
    ...
    while (elem != NULL) { /// elenco tutti i nodi nella lista
        ...
        /// espando arco  n --> elem->val
        /// quindi DFS(elem->val)
        //Se l'arco non è sotto la soglia allora lo scartiamo dato che è un arco leggero e non ci interessa
        if(elem->w < limit){
            output_graph << "dfs_" << n << " -> dfs_" << elem->val;
            if (V_visitato[elem->val])
                output_graph << "[color=gray, label = \"" << ct_visit++ << "\"]";
...
int main(int argc, char **argv) {
    ...
    for(int i = 0; i < 735; i++){
        ...
        //113 perchè era l'arco con peso più grande
        w = 113 - w;
    ...
    //Il codice commentato qui sotto cerca il percorso comune, per ogni nodo cerca l'arco più grande e ritorna la somma degli archi percorsi partendo dal nodo i,
    //Il percorso più grande risulterebbe il percorso comune, come alternativa si potrebbe misurare la somma di ogni singolo percorso ricorsivo e scegliere quello più alto come
    //rappresentante del percorso più comune partendo da un determinato nodo, in questo caso non cerco più l'arco più grande ma li prendo tutti e ricorsivamente calcolo il path fino a 
    //raggiungere un nodo foglia, si confrontano i percorsi e quello più grande sarà il mio rappresentante per il nodo i.
    // int best_path = 0;
    // int best_index;
    // int path_weight;
    ...
    //Eseguo uno shortest path in modo da creare i percorsi più pesanti del nodo i
    for(int i = 0; i < n_nodi; i++){

        shortest_path(i);

        //Creazione lista di adiacenza
        ...
        //Inseriamo i nodi nella lista di adiacenza con le informazioni ottenute da shortest path
        for(int j = 0; j < n_nodi; j++){
            if(V_prev[j] == -1){
        ...
        //Impostiamo di nuovo le distanze ad infinito così possiamo lavorare allo shortest path che parte dal prossimo nodo
        for(int j = 0; j < n_nodi; j++){
        ...
        //Eseguo questa funzione per disegnare e trovare i percorsi che vengono eseguiti da un buon numero di studenti, la soglia ci darà il numero giusto di studenti per
        //cui un percorso può essere considerato interessante
